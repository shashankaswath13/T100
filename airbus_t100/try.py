...
class FlexibleKivyApp(App):
    ...
    def process_pdf_for_checkpoint_7(self, pdf_path):
        ...
        # ✅ DB Matching (handle multiple actual fasteners)
        matched = False
        try:
            conn = sqlite3.connect(db_path)
            cur = conn.cursor()
            for n, a in zip(nominal_data, actual_data):
                cur.execute("SELECT [Fastener 1], [Fastener 2] FROM Sheet1 WHERE [Hole code1] = ?", (n['HOLE CODE'],))
                row = cur.fetchone()
                print(f"\n🔍 Query for Nominal HOLE CODE {n['HOLE CODE']} returned: {row}")

                if row:
                    db_fasteners = [f.strip().upper() for f in row if f]

                    actual_hole_code = a['HOLE CODE'].strip().upper()
                    nominal_hole_code = n['HOLE CODE'].strip().upper()
                    actual_first = a['FASTENERS'].split("+")[0].strip().upper()

                    # ✅ Combined check: Nominal vs Actual hole code + first fastener
                    if actual_hole_code == nominal_hole_code and actual_first in db_fasteners:
                        print(f"   ✅ HOLE CODE '{actual_hole_code}' matched Nominal '{nominal_hole_code}' "
                            f"and First Fastener '{actual_first}' matched in DB {db_fasteners}")
                        matched = True
                    else:
                        print(f"   ❌ Mismatch → HOLE CODE: actual={actual_hole_code}, nominal={nominal_hole_code} "
                            f"| Fastener: '{actual_first}' not in {db_fasteners}")
                else:
                    print("   ❌ Nominal HOLE CODE not found in DB")
        ...
        self.update_status_cell(6, "Matched" if matched else "Not Matched",
    ...
    def process_pdf_for_checkpoint_9(self, pdf_path):
        ...
        # 5️⃣ DB Matching (check each actual pair against nominal DB entry)
        matched = False
        try:
            conn = sqlite3.connect(db_path)
            cur = conn.cursor()
            ...
            for n, a in zip(nominal_data, actual_data):
                ...
                if row:
                    ...
                    # Split actual into fastener + collar
                    parts = [p.strip() for p in a['FASTENERS_COLLARS'].split("+")]
                    if len(parts) == 2:
                        actual_fastener, actual_collar = parts
                    else:
                        print("   ❌ Invalid fastener+collar format")
                        continue
                    ...
                    if actual_fastener in fasteners_db and actual_collar in collars_db:
                        print("   ✅ Matched in DB")
                        matched = True
                    else:
